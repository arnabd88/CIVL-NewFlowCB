...
public interface EnablerIF<STATE, TRANSITION, TRANSITIONSEQUENCE> {
	...
	TRANSITIONSEQUENCE enabledTransitionCB(STATE source, int currProc, boolean getCB);
	TRANSITIONSEQUENCE enabledTransitionForThisProcess(STATE source , int pid);
	TRANSITIONSEQUENCE enabledTransitionNotForThisProcess(STATE source, int pid);
	int getNswitch(STATE source);
	boolean  haveEnabledTransForThisProcess(STATE source , int pid);
	...
	//--- To get the sizze of the transitionSequence);
	int size(TRANSITIONSEQUENCE sequence);

	int basePid(TRANSITION transition);
