...
public abstract class CommonEnabler implements Enabler {
	...
	public TransitionSequence enabledTransitions(State state) {
		...
		//System.out.println("Transitions = " + enabledTransitionsOfAllProcesses(state) + "\n");
		if (transitions == null)
...
//	 }
	...
	@Override
	public int basePid( Transition transition) {
    ...
    //----------- For all enabled -------
	@Override
	public int getNswitch(State state) {
	   return state.getNswitch();
	}

	@Override
	public TransitionSequence  enabledTransitionCB(State state, int currentProcess, boolean getCB) {
	...
	//	List<Transition> tempTrans = enabledTransitionsOfProcess(state, currentProcess, currentProcess);
	//	if(!tempTrans.isEmpty()) transitions.addAll(tempTrans);
		//transitions.addAll(this.enabledTransitionsOfProcess(state, currentProcess, currentProcess));
		for(ProcessState process : processes) {
		   if(getCB == true)// && process.getPid()!=currentProcess)
			   transitions.addAll(this.enabledTransitionsOfProcess(state, process.getPid(), currentProcess)) ;
		   else if(process.getPid() == currentProcess)
		  	   transitions.addAll(this.enabledTransitionsOfProcess(state, process.getPid(), currentProcess)) ;
		}
	...
	public TransitionSequence enabledTransitionForThisProcess(State state, int processId) {
	    TransitionSequence resultSeq = Semantics.newTransitionSequence(state, true);
		resultSeq.addAll(this.enabledTransitionsOfProcess(state, processId));
		return resultSeq ;
	}
	...
	public TransitionSequence enabledTransitionNotForThisProcess(State state, int processId) {
		...
		TransitionSequence result = Semantics.newTransitionSequence(state, true);
		for ( ProcessState process : processes) {
			 //System.out.println("Looping process : " + process.getPid());
	...
	TransitionSequence enabledTransitionsOfAllProcesses(State state) {
        ...
        //System.out.println("Processes : " + state.getProcessStates());
		//System.out.println("NewTransitionSequence = " + Semantics.newTransitionSequence(state,true));
        //System.out.println("New For Loop @State = " + state) ;
		for (ProcessState process : processes) {
		    //System.out.println(" Process P = " + process + "  : PID = " + process.getPid());
			//System.out.println(" EnabledTransitions = " + this.enabledTransitionsOfProcess(state, process.getPid())) ;
			transitions.addAll(this.enabledTransitionsOfProcess(state,
					process.getPid()));
		}
		result.addAll(transitions);
		//System.out.println("Result-Transition-Sequence = " + result + "\n\n");
		//System.out.println("Using peek : " + result.peek() + "\n");
		//System.out.println("peek pid : " + result.peek().pid() + "\n");
		//System.out.println("peek processIdentifier : " + result.peek().processIdentifier() + "\n");
		//System.out.println("peek state : " + result.state() + "\n");
		return result;
	}
	...
	List<Transition> enabledTransitionsOfProcess(State state, int pid,
		...
		//System.out.println("ProcessState p = " + p);
	...
	//---- Overloaded
	List<Transition> enabledTransitionsOfProcess(State state, int pid,
			Map<Integer, Map<Statement, BooleanExpression>> newGuardMap, int currProc) {
		ProcessState p = state.getProcessState(pid);
		//System.out.println("ProcessState p = " + p);
		//System.out.println("PID = " + pid);
		Location pLocation = p.getLocation();
		...
		for (int i = 0; i < numOutgoing; i++) {
			...
			if (!newPathCondition.isFalse()) {
				transitions.addAll(enabledTransitionsOfStatement(state,
						statement, newPathCondition, pid, atomicLockAction, currProc));
			}
		}
		return transitions;
	}
	...
	private TransitionSequence enabledAtomicTransitions(State state) {
		...
		//System.out.println("PidInAtomic = " + pidInAtomic);
		if (pidInAtomic >= 0) {
	...
	private List<Transition> enabledTransitionsOfStatement(State state,
			Statement statement, BooleanExpression pathCondition, int pid,
			AtomicLockAction atomicLockAction, int currProc) {
		List<Transition> localTransitions = new LinkedList<>();
		int processIdentifier = state.getProcessState(pid).identifier();
		...
		try {
			...
			localTransitions.add(Semantics.newTransition(pathCondition, pid,
					processIdentifier, statement, atomicLockAction, currProc));
		} catch (UnsatisfiablePathConditionException e) {
			// nothing to do: don't add this transition
		}
		return localTransitions;
	...
	public List<Transition> enabledTransitionsOfProcess(State state, int pid, int currProcess) {
		return this.enabledTransitionsOfProcess(state, pid,
				new HashMap<Integer, Map<Statement, BooleanExpression>>(0), currProcess);
	}
