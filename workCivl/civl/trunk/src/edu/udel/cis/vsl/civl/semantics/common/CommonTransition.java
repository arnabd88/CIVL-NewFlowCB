...
public class CommonTransition implements Transition {
	...
	private boolean isContextSwitch = false ;
	...
	public CommonTransition(BooleanExpression pathCondition, int pid,
			int processIdentifier, Statement statement, boolean symplifyState,
			AtomicLockAction atomicLockAction, int currProcess) {
		this.pathCondition = pathCondition;
		this.pid = pid;
		this.statement = statement;
		this.processIdentifier = processIdentifier;
		this.atomicLockAction = atomicLockAction;
		this.simplifyState = symplifyState;
		if(currProcess == pid) setContextSwitch(false);
		else setContextSwitch(true);
	}
	...
	@Override
	public void setContextSwitch(boolean flag_cs) {
	    this.isContextSwitch = flag_cs ;
	}
